#include "nano/analysis/interface/slmassAnalysis.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
slmassAnalysis::slmassAnalysis(TTree *tree, Bool_t isMC, Bool_t sle, Bool_t slm) : semiLepTopAnalysis(tree, isMC, sle, slm)
{
}

slmassAnalysis::~slmassAnalysis()
{
}

void slmassAnalysis::Loop() {
  if (fChain == 0) return;
...
int main(int argc, char* argv[]) {
  ...
  if (argc != 1) {
    ...
    for(Int_t i = 3; i < argc; i++) {
      ...
      slmassAnalysis t(tree, isMC, isSL_e, isSL_m);
    ...
    slmassAnalysis t(tree, true, false, false);
...
void slmassAnalysis::collectTMVAvalues() {
...
void slmassAnalysis::setOutput(std::string outputName) {
  ...
  //bdtg->BookMVA("BDTG", "/cms/scratch/yyoun/nanoAOD/src/nano/analysis/test/topMass/cut/tmva/dataset/weights/TMVAClassification_BDTG.weights.D0.xml");
...
void slmassAnalysis::MakeBranch(TTree* t) {
  ...
  m_tree->Branch("lep_pid", &b_lep_pid, "lep_pid/I");    
  ...
  //t->Branch("cme_angleXY", &b_cme_angleXY, "cme_anbleXY/F");
  //t->Branch("cme_angleXYZ", &b_cme_angleXYZ, "cme_anbleXYZ/F");
...
void slmassAnalysis::resetBranch() {
  d0s.clear();
...
void slmassAnalysis::cmesonSelection() {
  if (nhad < 1) return;
