...
void vtsAnalyser::MakeBranch(TTree* t) {
  ...
  BranchVF(GenPart_x_j_vec); BranchVF(GenPart_dr_j_vec); BranchVO(GenPart_isInSJet_j_vec);
  BranchVF(GenPart_x_gj_vec); BranchVF(GenPart_dr_gj_vec); BranchVO(GenPart_isInSJet_gj_vec);
  ...
  BranchVF(hadTruth_x_j_vec); BranchVF(hadTruth_dr_j_vec); BranchVO(hadTruth_isInSJet_j_vec);
  BranchVF(hadTruth_x_gj_vec); BranchVF(hadTruth_dr_gj_vec); BranchVO(hadTruth_isInSJet_gj_vec);
...
void vtsAnalyser::ResetBranch() {
  ...
  b_GenPart_x_j_vec.clear(); b_GenPart_dr_j_vec.clear(); b_GenPart_isInSJet_j_vec.clear();
  b_GenPart_x_gj_vec.clear(); b_GenPart_dr_gj_vec.clear(); b_GenPart_isInSJet_gj_vec.clear();
  ...
  b_hadTruth_x_j_vec.clear(); b_hadTruth_dr_j_vec.clear(); b_hadTruth_isInSJet_j_vec.clear();
  b_hadTruth_x_gj_vec.clear(); b_hadTruth_dr_gj_vec.clear(); b_hadTruth_isInSJet_gj_vec.clear();
...
void vtsAnalyser::Test() {
  for (unsigned int i=0; i<nGenPart; ++i) {
    ...
    float drReco = 99; int drIdxReco = -1;
    float drGen = 99; int drIdxGen = -1;
    ...
    for (unsigned int j=0; j<nJet; ++j) { // Loop for all of recoJet
      ...
      if (dr < drReco ) { 
        drReco = jet_tlv.DeltaR(gen_tlv);
        drIdxReco = j;
      } /*else if (dr == drReco) { // not completed
        drReco = jet_tlv.DeltaR(gen_tlv);
        drIdxReco = j;
      }*/
    }
    for (unsigned int j=0; j<nGenJet; ++j) { // Loop for all of genJet
      ...
      auto dr = jet_tlv.DeltaR(gen_tlv);
      if (dr < drGen ) {           
        drGen = jet_tlv.DeltaR(gen_tlv);
        drIdxGen = j;
      } /*else if (dr == drGen) { // not completed
        drGen = jet_tlv.DeltaR(gen_tlv);
        drIdxGen = j;
      }*/
    } 
    ...
    b_GenPart_dr_gj_vec.push_back(drGen);
    b_GenPart_x_gj_vec.push_back(GenPart_pt[i]/Jet_pt[drIdxGen]);
    b_GenPart_isInSJet_gj_vec.push_back(isInGenJetS);
  }

  std::vector<std::pair<unsigned int, float>> cReco;
  std::map<unsigned int, int> cRecoJet;
  for (unsigned int i=0; i<nhad; ++i) {
    ...
    b_hadTruth_dr_gj_vec.push_back(drGen);
    b_hadTruth_x_gj_vec.push_back(had_pt[i]/Jet_pt[drIdxGen]);
    b_hadTruth_isInSJet_gj_vec.push_back(isInGenJetS);
  }
/*
  std::sort(cReco.begin(), cReco.end(), [] (std::pair<unsigned int, float> a, std::pair<unsigned int, float> b) { return (a.second < b.second); } );
  auto idx = cReco[0].first;
  auto jetIdx = cRecoJet[idx];
*/ // not completed
}
