...
//   https://github.com/cms-sw/cmssw/blob/CMSSW_9_4_X/CondFormats/JetMETObjects/src/SimpleJetCorrectionUncertainty.cc
//   in d86f5ed commit
...
// MODIFIED
namespace edm {
  std::ostream &LogError(const char *szTitle) {return std::cerr << szTitle << ": ";}
};
...
float SimpleJetCorrectionUncertainty::uncertaintyBin(unsigned fBin, float fY, bool fDirection) const 
{
  ...
  if ((p.size() % 3) != 0)
    ...
    std::stringstream(strException) << "SimpleJetCorrectionUncertainty: " 
      <<"wrong # of parameters: multiple of 3 expected, "<<p.size()<< " got" << std::endl;
