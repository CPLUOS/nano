...
//   https://github.com/cms-sw/cmssw/blob/CMSSW_9_4_X/CondFormats/JetMETObjects/src/JetCorrectorParameters.cc
//   in 4d7b6ef commit
...
//#include "CondFormats/JetMETObjects/interface/JetCorrectorParametersHelper.h" // MODIFIED
#include "nano/external/interface/JetCorrectorParameters.h" // MODIFIED
#include "nano/external/interface/JetCorrectorParametersHelper.h" // MODIFIED
#include <iostream>
#include <iomanip>
#include <fstream>
#include <sstream>
...
JetCorrectorParameters const & JetCorrectorParametersCollection::operator[]( key_type k ) const {
  ...
  std::stringstream(strException) << "InvalidInput: "
                                  << " cannot find key " << static_cast<int>(k)
                                  << " in the JEC payload, this usually means you have to change the global tag" << std::endl;
  throw std::runtime_error(strException);
}
...
JetCorrectorParametersCollection::findKey( std::string const & label ) const {
  ...
  std::string strException;
  std::stringstream(strException) << "InvalidInput: " << " Cannot find label " << label << std::endl;
  throw std::runtime_error(strException);
}
...
//#include "FWCore/Utilities/interface/typelookup.h" // MODIFIED

//TYPELOOKUP_DATA_REG(JetCorrectorParameters); // MODIFIED
//TYPELOOKUP_DATA_REG(JetCorrectorParametersCollection); // MODIFIED
