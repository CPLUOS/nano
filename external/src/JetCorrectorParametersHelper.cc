...
//   https://github.com/cms-sw/cmssw/blob/CMSSW_9_4_X/CondFormats/JetMETObjects/src/JetCorrectorParametersHelper.cc
//   in 4d7b6ef commit
  ...
  for (unsigned idim=1; idim<=N; idim++) {
    ...
    for (unsigned iRecord=0; iRecord<nRec; iRecord++) {
      if (mRecords[iRecord].xMin(idim)!=mBinBoundaries[idim][nBoundaryPassed%fN[idim-1]]) {
        ...
        std::stringstream(strException) << "MissingRecord: "
